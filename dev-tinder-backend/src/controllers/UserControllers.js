...
export const signin = async (req, res) => {
  ...
  // httpOnly: true this will make the cookie not accessible from the client side
  // secure will be true on production cause secure true means the cookie will only be sent over HTTPS
  res.cookie("token", token, {
    expires: new Date(Date.now() + 12 * 3600000),
    httpOnly: true,
    secure: config.nodeEnv === "production",
  }); // cookie will expire in 12 hours
...
export const getAllUsers = async (req, res) => {
  const users = await UserModel.find({}).select(
    "name photoUrl age gender about skills",
  );
  return res
    .status(200)
...
export const pendingConnectionRequests = async (req, res) => {
  const loggestInUser = req.user;
  const connectionRequests = await ConnectionRequestModel.find({
    recieverId: loggestInUser._id,
    status: "interested",
...
export const connections = async (req, res) => {
  ...
  const connectionRequests = await ConnectionRequestModel.find({
    ...
    .populate({ path: "senderId", USER_SAFE_DATA})
    .populate({ path: "recieverId", USER_SAFE_DATA });
